...
const plantWiseTripReportParameters = {
  mode: 2,
  rampid: 1,
...
const populateTable = async () => {
  try {
    dateSpan.innerHTML = `Plant Wise Trip Report:- From ${formatDate(
      plantWiseTripReportParameters.date1
    )} To ${formatDate(plantWiseTripReportParameters.date2)}`;
    ...
    const plantWiseTripReportRequestData = {
      storedProcedureName: "proc_IncomingTransaction",
      parameters: JSON.stringify(plantWiseTripReportParameters),
    };

    const response = await axios.post(
      commonApi,
      plantWiseTripReportRequestData
    );
...
const handleRampSelect = (event) => {
  ...
  plantWiseTripReportParameters.rampid = selectedValue;
};

const handleStartDateSelect = () => {
  selectedStartDate = startDate.value;
  plantWiseTripReportParameters.date1 = startDate.value;
};

const handleEndDateSelect = () => {
  selectedEndDate = endDate.value;
  plantWiseTripReportParameters.date2 = endDate.value;
};
