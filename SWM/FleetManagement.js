...
const geoJsonFile = "./service/geofence.txt";
...
const populateLiveVehicleTypeDropdown = async () => {

    try {
        const vehicleType = document.getElementById("ddlVehicletype");
        
        // Check if element exists before proceeding
        if (!vehicleType) {
            console.warn("Vehicle type dropdown element not found. Retrying after delay...");
            // Retry after a short delay to handle cases where DOM is not fully loaded
            setTimeout(() => {
                populateLiveVehicleTypeDropdown();
            }, 500);
            return;
        }

        // Check if commonApi is defined
        if (typeof commonApi === 'undefined' || !commonApi) {
            console.error("commonApi is not defined. Cannot fetch vehicle types.");
            // Don't show error popup if API is not configured - just log it
            return;
        }
        ...
        // Check if response and data structure exist
        if (!response || !response.data || !response.data.CommonMethodResult) {
        ...
        if (!returnValue) {
            console.warn("No vehicle type data returned from API");
            return;
        }
        ...
        if (!Array.isArray(dropdownData)) {
            console.warn("Vehicle type data is not an array");
            return;
        }

        for (const item of dropdownData) {
            if (item && item.vehicleType) {
                const option = document.createElement("option");
                option.value = item.vehicleType; // use name directly
                option.text = item.vehicleType;
                vehicleType.appendChild(option);
            }
        }

        if (vehicleType.fstdropdown && typeof vehicleType.fstdropdown.rebind === 'function') {
            vehicleType.fstdropdown.rebind();
        }
    }
    catch (error) {
        console.error("Error fetching vehicle type data:", error);
        // Only show error if element exists (to avoid popup on mobile when element is hidden)
        const vehicleType = document.getElementById("ddlVehicletype");
        if (vehicleType) {
            Swal.fire("Error", "Failed to load vehicle type.", "error");
        }
    }
};
...
//  console.log("Vehicle Type Selected:", selectedType); // Log selected value

//  // Clear and disable vehicle dropdown initially
//  const liveVehicleDropdown = document.getElementById("liveVehicleDropdown");
//  liveVehicleDropdown.innerHTML = '<option value="0">Select</option>';
//  liveVehicleDropdown.disabled = true;

//  if (selectedType !== "0" && selectedType !== "") {
//    console.log("Calling populateLiveVehicleDropdown with type:", selectedType);
//    populateLiveVehicleDropdown(selectedType);
//    console.log("Calling getGroupTrackData with type:", selectedType);
//    getGroupTrackData(selectedType);
//  }
//  else
//  {
//    console.warn("Invalid or default vehicle type selected. Dropdown will remain disabled.");
//  }
//};
...
const handleVehicleTypeChange = () => {
    ...
    if (!vehicleTypeDropdown) {
        console.warn("Vehicle type dropdown not found (mobile version)");
        return;
    }
    ...
    const liveVehicleDropdown = document.getElementById("liveVehicleDropdown");
    if (!liveVehicleDropdown) {
        console.warn("Live vehicle dropdown not found");
        return;
    }
    ...
    if (selectedType !== "0" && selectedType.trim() !== "") {
        console.log("Selected Vehicle Type:", selectedType);

        // Optionally fetch specific vehicles for dropdown (if needed elsewhere)
        populateLiveVehicleDropdown(selectedType);

        // Trigger group tracking with selected vehicle type
        getGroupTrackData(selectedType);
    } else {
        console.warn("Invalid vehicle type selected.");
    }
};



// Load vehicle dropdown (based on selected type)
const populateLiveVehicleDropdown = async (vehicleTypeName = "") => {
    try {
        console.log("Selected Vehicle Type:", vehicleTypeName);

        const liveVehicleDropdown = document.getElementById("liveVehicleDropdown");
        if (!liveVehicleDropdown) {
            console.warn("liveVehicleDropdown element not found");
            return;
        }
        
        liveVehicleDropdown.innerHTML = '<option value="0">Select</option>';

        // Get zone and ward values (use 0 if not selected)
        const zoneId = zoneDropdown && zoneDropdown.value ? (zoneDropdown.value === "0" ? 0 : Number(zoneDropdown.value)) : 0;
        const wardId = wardDropdown && wardDropdown.value ? (wardDropdown.value === "0" ? 0 : Number(wardDropdown.value)) : 0;

        const requestData = {
            storedProcedureName: "Proc_VehicleMap_1",
            parameters: JSON.stringify({
                Mode: 22,
                UserID: loginId || 11401,
                RoleID: 4,
                zoneId: zoneId,
                WardId: wardId,
                zoneName: "default",
                WardName: "default",
                vehicletypename: vehicleTypeName || "", // pass vehicle type as string (empty if not provided)
            }),
        };
        ...
        liveVehicleDropdown.disabled = false;
...
const populateHistoryVehiDropdown = async () => {
    try {
        if (!historyVehicleDropdown) {
            console.warn("historyVehicleDropdown element not found");
            return;
        }
...
//const getGroupTrackData = async (vehicleTypeName) => {
//  try {
//    console.log("Selected Vehicle Type for group tracking :", vehicleTypeName);
//        const requestData = {
//            storedProcedureName: "Proc_VehicleMap_1",
//            parameters: JSON.stringify({
//                mode: 24,
...
//    }
//};
// 3. Get group tracking data from backend
//const getGroupTrackData = async (vehicleTypeName) => {
//  try {
//    console.log("Fetching group tracking data for type:", vehicleTypeName);
...
//        RoleID: 4,
//          vehicletypename: vehicleTypeName
///*        vehicletypename: "Bolero"*/
//      }),
//    };

//    const response = await axios.post(commonApi, requestData);
//    const list = response.data.CommonMethodResult.CommonReportMasterList;

//    if (!list || !list[0]?.ReturnValue) {
//      toast.show("No data found for selected vehicle type");
//      return;
//    }

//    const data = JSON.parse(list[0].ReturnValue);
//    console.log("Group tracking data:", data);

//    addGroupVehicles(data);
//  } catch (error) {
//    console.error("Error fetching group tracking data:", error);
//  }
//};
const getGroupTrackData = async (vehicleTypeName) => {
    try {
        // Normalize empty dropdown values to 0 (backend expects numeric values)
        const zoneId = zoneDropdown.value === "" || zoneDropdown.value === "0" ? 0 : Number(zoneDropdown.value);
        const wardId = wardDropdown.value === "" || wardDropdown.value === "0" ? 0 : Number(wardDropdown.value);
        const vendorId = vendorDropdown.value === "" || vendorDropdown.value === "0" ? 0 : Number(vendorDropdown.value);
        ...
        const requestData = {
            ...
            parameters: JSON.stringify({
                ...
                vendorid: vendorId,
                RoleID: 4,
                vehicletypename: vehicleTypeName || "", // Include vehicle type if provided
            }),
        };
        ...
        if (error.response) {
            console.error(
                "Error fetching group track data:",
                error.response.status,
                error.response.statusText,
                error.response.data
            );
        } else {
            console.error("Error fetching group track data:", error.message || error);
        }
        toast.show();
    }
};
...
const handleZoneSelect = async (event) => {
    ...
    await populateLiveVehicleDropdown(); // Works without vehicle type (mobile compatible)
    await populateHistoryVehiDropdown();
    // Only populate vehicle type dropdown if it exists (desktop version)
    const vehicleTypeDropdown = document.getElementById("ddlVehicletype");
    if (vehicleTypeDropdown) {
        await populateLiveVehicleTypeDropdown();
    }
};
...
const showGeofence = (gdata) => {
    removeLiveVehicle(); //Removing live vehicle from the map
    removeGropTrackingVehicles(); //Removing group tracking vehicles from the map
...
const addMarkersWithDelay = (data) => {
    ...
    console.log("gps location data is here", data);

    for (var i = 0; i < data.length; i++) {
        var lat = data[i].latitude;
        var lon = data[i].longitude;
        var time = `<b>Date:</b> ${data[i].datetim.split("T")[0]} <br> <b>Time:</b> ${
            data[i].datetim.split("T")[1]
            }`;
        var speed = data[i].speed;

        marker1 = L.marker([lat, lon], { icon: dot }).addTo(dotMarkerLayer);
        marker1.bindPopup(`${time} <br> <b>Speed:</b> ${speed} <br> <b>Latitude:</b> ${data[i].latitude} <br> <b>Longitude:</b> ${data[i].longitude}`);

        if (i === 0) {
            // This is the start point
            startMarker = L.marker([lat, lon], { icon: greenflag }).addTo(
                dotMarkerLayer
            );
            startMarker.bindPopup(time);
        } else if (i === data.length - 1) {
            // This is the end point
            endMarker = L.marker([lat, lon], { icon: redflag }).addTo(dotMarkerLayer);
...
btnGroupPlay.addEventListener("click", () => {
    ...
    removeLiveVehicle();
    if (!isFetching) {
        // Get current vehicle type from dropdown dynamically on each interval call
        // If dropdown doesn't exist (mobile version), use empty string
        fetchInterval = setInterval(() => {
            const vehicleTypeDropdown = document.getElementById("ddlVehicletype");
            const currentVehicleType = vehicleTypeDropdown && vehicleTypeDropdown.value !== "0" ? vehicleTypeDropdown.value : "";
            getGroupTrackData(currentVehicleType);
        }, 5000);
...
const removeGropTrackingVehicles = () => {
    if (featureGroup) {
        map.removeLayer(featureGroup);
    }
    if (featureGroupw) {
...
window.addEventListener("load", async () => {
    ...
    // Only populate vehicle type dropdown if it exists (desktop version)
    const vehicleTypeElement = document.getElementById("ddlVehicletype");
    if (vehicleTypeElement) {
        await populateLiveVehicleTypeDropdown();
        vehicleTypeElement.addEventListener("change", handleVehicleTypeChange);
    } else {
        // Mobile version - populate live vehicle dropdown without vehicle type filter
        console.log("Mobile version detected - vehicle type dropdown not found");
        await populateLiveVehicleDropdown();
    }
    await populateHistoryVehiDropdown();
