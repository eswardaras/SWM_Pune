...
const getData = async () => {
  try {
    dateSpan.innerHTML = `Ramp Summary Report:- From ${formatDate(
      startDate.value
    ...
    const requestData = {
      storedProcedureName: "sp_RampSummaryReport",
    ...
    let tableDataRamps = JSON.parse(rampData);
    let tableDataTotals = JSON.parse(totalData);

    console.log("Ramp Data", tableDataRamps);
    console.log("Totals", tableDataTotals);
    
    function isFullMonthOrMore(start, end) {
    // Clone dates (so we don't modify original sData / eData)
    ...
    // MUST start on 1st of a month
    if (s.getDate() !== 1) return false;

    // Last day of the start month
    const endOfStartMonth = new Date(s.getFullYear(), s.getMonth() + 1, 0);

    // ✅ Allow if end date covers at least the whole start month
    return e >= endOfStartMonth;
}

if (isFullMonthOrMore(sData, eData))
    ...
    //if ((eData - sData) / (1000 * 60 * 60 * 24) >= 30)
 {
      ...
      zip.file(
        ...
        s2ab(createExcelFile(tableDataRamps, "Ramps")),
        { binary: true }
      );
      zip.file(
      ...
      populateRampTable(tableDataRamps);
...
// Populate Ramp Table
const populateRampTable = (tableData) => {
    ...
    buttons: [
      {
        ...
        action: function () {
          exportExcel(tableId, "Ramp Summary Report");
        }
      },
      {
        ...
        action: function () {
          exportPDF(tableId, "Ramp Summary Report");
        }
      }
    ],
    language: {
...
function exportPDF(tableId, title = "Ramp Summary Report") {
...
function exportExcel(tableId, title = "Ramp Summary Report") {
  // Get table element
