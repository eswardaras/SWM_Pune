    ...
    try {
        ...
        const requestData = {
            ...
            parameters: JSON.stringify({
                ...
                zoneName: "default",
                WardName: "default",
                vehicletypename: vehicleTypeName, // pass vehicle type as string
            }),
        };
...
btnGroupPlay.addEventListener("click", () => {
    ...
    if (!isFetching) {
        ...
        fetchInterval = setInterval(() => {
            const vehicleTypeDropdown = document.getElementById("ddlVehicletype");
            const currentVehicleType = vehicleTypeDropdown ? vehicleTypeDropdown.value : "";
...
window.addEventListener("load", async () => {
    ...
    if (vehicleTypeElement) {
        ...
        console.warn("Vehicle type dropdown element not found for event listener. Retrying after delay...");
        // Retry after a short delay
        setTimeout(() => {
            const retryElement = document.getElementById("ddlVehicletype");
            if (retryElement) {
                retryElement.addEventListener("change", handleVehicleTypeChange);
            }
        }, 500);
    }
