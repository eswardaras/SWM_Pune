...
const populateRampDropdown = async () => {
    try {
        const ddlRamp = document.getElementById("ddlPlant");
...
//        const ddlPlant = document.getElementById('ddlPlant');

//        const requestData = {
//            storedProcedureName: "proc_Plant_Get_Data_FOR_WAIT_TIME_CALCULATION",
//            parameters: JSON.stringify({
//                mode: 2,
//                ramp_id: ddlPlant.value,
...
const populateDataGrid = async () => {
    ...
    try {
        ...
        // Table 1: Main data
        const tableData = JSON.parse(commonReportMasterList[0].ReturnValue);
        console.log("Parsed Table Data:", tableData);
        ...
        // Table 2: Average wait time (assumed to be 1 row with WaitTime column)
        const avgData = JSON.parse(commonReportMasterList[1].ReturnValue);
        const avgTime = avgData.length > 0 ? avgData[0].AvgWaitTime : "N/A";

        // Update label
        const label = document.getElementById("dataStatusLabel");
        label.textContent = `Average Wait Time: ${avgTime}`;
        label.className = "badge bg-primary text-white ms-3";
        ...
        console.error("Error fetching data:", error);
        const label = document.getElementById("dataStatusLabel");
        label.textContent = "Error fetching wait time";
        label.className = "badge bg-danger text-white ms-3";
    }
};
