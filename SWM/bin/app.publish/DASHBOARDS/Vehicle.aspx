<%@ Page Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Vehicle.aspx.cs" Inherits="SWM.Vehicle" %>
            ...
            <div class="filter-bar">
                ...
                <div class="filter-group">
                    <select id="selectWard">
                        <option value="all">Ward (leave blank to include all)</option>
                    </select>
                </div>

                <div class="filter-group">
                    <select id="selectVehicleName">
                        <option value="all">Vehicle Name (leave blank to include all)</option>
                    </select>
                </div>

                <div class="filter-group">
                    <select id="selectVendorName">
                        <option value="all">Vendor Name (leave blank to include all)</option>
                    </select>
                </div>

                <div class="filter-group">
                    <select id="selectVehicleStatus">
                        <option value="all">Vehicle Status (leave blank to include all)</option>
                    </select>
                </div>

                <div class="filter-group">
                    <select id="selectVehicleType">
                        <option value="all">Vehicle Type (leave blank to include all)</option>
                    </select>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    function renderVehicleStatus() {
        ...
        $.ajax({
            type: "GET",
            url: "/api/chartdashboard/VehicleStatus",
            dataType: "json",

            success: function (res) {
                toggleLoader(chartId, false);
                if (!res || !Array.isArray(res.datasets)) {
                    console.error("Invalid VehicleStatus API response");
                    return;
                }
                ...
                res.datasets.forEach(ds => {
                    ...
                    if (sum > 0) {  // hide zero bars
                        xLabels.push(ds.label === "Breakdown" ? "Break" : ds.label);
                        yValues.push(sum);
                        textValues.push(sum.toString());
                        barColors.push(colorMap[ds.label] || "#999");
                        totalVehicles += sum;
                    }
                });

                if (xLabels.length === 0) {
                    console.error("No vehicle status data to display");
                    return;
                }
                ...
                /* ------------------ Y-AXIS RANGE ------------------ */
                const maxValue = Math.max(...yValues) + 50;

                /* ------------------ LAYOUT ------------------ */
                let layout = getBaseLayout(`Vehicle Status (Total Vehicles : ${totalVehicles})`);
                ...
                layout.margin = { l: 50, r: 30, t: 50, b: 60 };
                layout.showlegend = false;

                /* ------------------ RENDER ------------------ */
                Plotly.newPlot("vehicleStatusChart", [trace], layout, {
                    responsive: true,
    ...
    function renderZoneWiseVehicleStatus() {
        ...
        $.ajax({
            type: "GET",
            url: "http://localhost:75/api/ZoneWiseVehicleStatus",
    ...
    function renderVendorWiseVehicleStatus() {
        ...
        $.ajax({
            type: "GET",
            url: "http://localhost:75/api/VendorwiseVehicleStatus",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
    ...
    function renderVehicleTypeWiseStatus() {
        ...
        $.ajax({
            type: "GET",
            url: "http://localhost:75/api/VehicleTypewiseStatus",
            dataType: "json",
