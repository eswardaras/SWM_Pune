...
const prabhagWiseWeightReportParameters = {
  mode: 2,
  rampid: 1,
...
const populateTable = async () => {
  try {
    dateSpan.innerHTML = `Prabhag Wise Weight Report:- From ${formatDate(
      prabhagWiseWeightReportParameters.date1
    )} To ${formatDate(prabhagWiseWeightReportParameters.date2)}`;
    ...
    const prabhagWiseWeightReportRequestData = {
      storedProcedureName: "proc_IncomingTransaction",
      parameters: JSON.stringify(prabhagWiseWeightReportParameters),
    };

    const response = await axios.post(
      commonApi,
      prabhagWiseWeightReportRequestData
    );
    ...
    const commonReportMasterList =
      response.data.CommonMethodResult.CommonReportMasterList[3].ReturnValue;
    ...
    for (const data of tableData) {
      ...
      tr.innerHTML = `<tr>
            <td>${data["Prabhag Name"]}</td>
            <td>${data["Total Trips"]}</td>
            <td>${data["Total Weight In (MT)"]}</td>
        </tr>`;
...
const handleRampSelect = (event) => {
  ...
  prabhagWiseWeightReportParameters.rampid = selectedValue;
};

const handleStartDateSelect = () => {
  selectedStartDate = startDate.value;
  prabhagWiseWeightReportParameters.date1 = startDate.value;
};

const handleEndDateSelect = () => {
  selectedEndDate = endDate.value;
  prabhagWiseWeightReportParameters.date2 = endDate.value;
};
